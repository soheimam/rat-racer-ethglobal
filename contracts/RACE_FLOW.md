...
const unwatch = publicClient.watchContractEvent({
  ...
  onLogs: (logs) => {
    logs.forEach((log) => {
      ...
      console.log(
        `🥇 1st: Rat #${winningRatTokenIds[0]} - ${formatEther(prizes[0])} RACE`
      );
      console.log(
        `🥈 2nd: Rat #${winningRatTokenIds[1]} - ${formatEther(prizes[1])} RACE`
      );
      console.log(
        `🥉 3rd: Rat #${winningRatTokenIds[2]} - ${formatEther(prizes[2])} RACE`
      );
