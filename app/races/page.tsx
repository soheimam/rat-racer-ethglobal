            ...
            <Dialog open={showCreateModal} onOpenChange={setShowCreateModal}>
                <DialogContent className="bg-black border-4 text-white max-w-2xl"
                    ...
                    <div className="space-y-6">
                        <div>
                            <label className="text-xs font-mono mb-2 block tracking-wider" style={{ color: '#718096' }}>ENTRY FEE (RACE TOKENS)</label>
                            <input
                                type="number"
                                value={newEntryFee}
                        ...
                        <button
                            ...
                            className="relative w-full px-16 py-6 font-mono font-black text-2xl tracking-wider transition-all duration-500 overflow-hidden group disabled:opacity-50 disabled:cursor-not-allowed hover:scale-105 disabled:hover:scale-100"
                            style={{
                                background: (isCreatingRace || pollingForRace) ? '#2d3748' : 'linear-gradient(to bottom right, #2d3748, #000000)',
                                color: '#cbd5e0'
                            }}
                        >
                            {!isCreatingRace && !pollingForRace && (
                                <>
                                    <div
                                        className="absolute inset-0 opacity-100"
                                        style={{
                            ...
                            {(isCreatingRace || pollingForRace) && (
                                <div className="flex items-center justify-center gap-3">
                                    <div className="w-6 h-6 border-2 border-t-transparent rounded-full animate-spin" style={{ borderColor: '#718096' }}></div>
                                    <span className="relative z-10">{isCreatingRace ? 'CONFIRMING TX' : 'PROCESSING'}</span>
                                </div>
                            )}
                        </button>
                    </div>
